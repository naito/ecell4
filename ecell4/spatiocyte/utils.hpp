...
inline const Real calculate_dimensional_factor(
        const VoxelPool* vp_a,
        ...
        const boost::shared_ptr<const SpatiocyteWorld>& world)
{
    ...
    const Real D_a(vp_a->D()), D_b(vp_b->D());
    const Shape::dimension_kind dim_a(vp_a->get_dimension()), dim_b(vp_b->get_dimension());
...
inline const Real calculate_alpha(
